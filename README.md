...
```
Dependabot                   semantic-release            docker-publish
  opens a PR bumping   ──►   on push to master:    ──►   on release published:
  FROM alpine:3.12           tag + GitHub Release        build & push to Docker Hub
                                                         (latest, 3, 3.2, 3.2.1)
```
...
newer Alpine tag is available, bumping `FROM alpine:3.12` for you. Merge the PR
into `master` and Part 2 takes over.
